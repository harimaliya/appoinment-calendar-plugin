...
function appointment_calendar_shortcode()
{
	...
	if(isset($_POST['booknowapp']))
	{
		...
		VALUES (NULL , '$name', '$email', '$serviceid', '$phone', '$start_time', '$end_time', '$appointmentdate', '$note', '$appointment_key', '$status', '$appointment_by');";
			...
			{
				echo "<div class='alert alert-success'><strong>Thank you for scheduling appointmnet with us.<br>A Confirmation mail will be forward to you soon after admin appoval.</strong></div>";
...
<?php
			...
			$FetchAllApps_sql = "select `name`, `start_time`, `end_time`, `date` FROM `wp_ap_appointments`";
...
<?php
			...
			$FetchAllEvent_sql = "select `name`, `start_time`, `end_time`, `start_date`, `end_date`, `repeat` FROM `wp_ap_events` where `repeat` = 'N'";
...
<?php
			...
			global $wpdb;
			$EventTableName = $wpdb->prefix."ap_events";
			$FetchAllREvent_sql = "select `name`, `start_time`, `end_time`, `start_date`, `end_date`, `repeat` FROM `wp_ap_events` where `repeat` != 'N'";
			...
			<form action="" method="post" name="addnewappointment" id="addnewappointment">
				...
				<div class="modal-body">
					...
					<?php
						global $wpdb;
						$ServiceTable = $wpdb->prefix."ap_services";
						$findservice_sql = "SELECT * FROM `$ServiceTable`";
...
<?php
	...
	<div class="modal" id="myModal" >
		...
		<div class="modal-body">
			...
			?>
			<?php //include(plugins_url('/menu-pages/time-slots-calculation.php', __FILE__)); ?>
			</div>
			<?php
			if(!$Enable)
			{
			...
			<hr size='5' />
					...
					<table>
					  <tr>
						<td width="15%" align="left" scope="row">Name</td>
					 ...
					 <tr>
					  ...
					  <tr>
						<td align="left" scope="row">Phone</td>
						<td align="center"><strong>:</strong></td>
						<td><input name="clientphone" type="text" id="clientphone" maxlength="12"/></td>
					  </tr>
					  <tr>
						<td align="left" valign="middle" scope="row">Note</td>
